...
Atlas atlas_prop_blue;
Atlas atlas_prop_pink;
Atlas atlas_prop_yellow;

IMAGE img_buff_icon_hurry;		// 疾跑 BUFF 图标
IMAGE img_buff_icon_invisible;	// 隐身 BUFF 图标
IMAGE img_buff_icon_recover;	// 恢复 BUFF 图标
IMAGE img_buff_icon_silence;	// 沉默 BUFF 图标
...
void load_game_resources()
{
	...
	// 道具动画图集加载
	atlas_prop_blue.load_from_file(_T("resources/buff_box_blue_%d.png"), 4);
	atlas_prop_pink.load_from_file(_T("resources/buff_box_pink_%d.png"), 4);
	atlas_prop_yellow.load_from_file(_T("resources/buff_box_yellow_%d.png"), 4);

	// 角色状态栏不同 BUFF 图标图片素材加载
	loadimage(&img_buff_icon_hurry, _T("resources/buff_icon_hurry.png"));
	loadimage(&img_buff_icon_invisible, _T("resources/buff_icon_invisible.png"));
	loadimage(&img_buff_icon_recover, _T("resources/buff_icon_recover.png"));
	loadimage(&img_buff_icon_silence, _T("resources/buff_icon_silence.png"));
...
int main(int argc, char** argv)
{
	...
	SetWindowText(hwnd, _T("植物明星大乱斗！- 进阶版"));
	...
	while (true)
	{
		...
		duration<double> delta = duration_cast<duration<double>>(start - last_time);
		
		scene_manager.on_update((int)(delta.count() * 1000));
		...
		last_time = start;
		nanoseconds sleep_duration = frame_duration - (steady_clock::now() - start);
